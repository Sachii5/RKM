...
router.get('/', authenticate, requireSupervisorOrAbove, async (req, res) => {
  try {
    const resUsers = await db.query('SELECT id, userid, fullname, role, first_login FROM users_local ORDER BY userid ASC');
...
router.post('/', authenticate, requireSupervisorOrAbove, async (req, res) => {
  ...
  try {
    ...
    await db.query(`
      ...
      VALUES ($1, $2, $3, $4, true)
    `, [upperUser, fullname || null, role.toUpperCase(), defaultHash]);
...
router.put('/:id', authenticate, requireSupervisorOrAbove, async (req, res) => {
  ...
  try {
    ...
    await db.query(`
      ...
      WHERE id = $4
    `, [upperUser, fullname || null, role.toUpperCase(), id]);
