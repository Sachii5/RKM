...
const getAlreadyZonedMemberCodes = async () => {
  const res = await db.query(`
    SELECT zm.member_code, COALESCE(bool_or(vl.visited = true AND vl.is_closed = false), false) as max_visited
    ...
    HAVING COALESCE(bool_or(vl.visited = true AND vl.is_closed = false), false) = true
  `);
